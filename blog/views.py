...
def addRecipeInfo(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            recipe.save()
            id = recipe.id            
            return render(request, 'blog/post_form.html', {'id':id, 'name':recipe.name, 'ingredients':recipe.ingredients})
    else:
...
def prof(request,username):
    ...
    posts = Recipe.objects.filter(author=user)
    # posts = Recipe.objects.filter()
    check = RecipeInstruction.objects.all()
    context = {"user":user,"posts":posts, "posts":posts}
